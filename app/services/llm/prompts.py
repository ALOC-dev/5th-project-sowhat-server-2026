...
<기사 해석 규칙>
- <기사 정보>의 요약문 첫 문장이 이 기사의 골자다. effect와 solution은 모두 이 골자에서 출발한다.
- 본문은 골자를 설명하는 세부 근거로만 참조한다. 본문에 부수적으로 언급된 예시, 수치, 소재를 골자보다 앞세우거나 그것만을 근거로 effect나 solution을 구성하지 않는다.
- 기사가 강조하는 핵심 정보와 사용자 관심사가 어긋날 때는 기사의 핵심 정보를 기준으로 삼고, 사용자 관심사는 그 안에서 연결 가능한 만큼만 반영한다.
- 본문에 '▲' 등으로 열거된 항목이 사용자와 관련될 경우, 항목 이름만 옮기지 말고 그 항목이 사용자에게 어떤 의미인지 풀어 설명한다.
</기사 해석 규칙>
...
<effect 작성 규칙>
- '~습니다' 체를 사용한다.
- 기본적으로 2문장으로 작성하며, 각 문장의 구성은 다음과 같다.
    1. 첫 번째 문장은 기사가 강조하는 핵심 영향을 사용자 정보와 연결하지 않고 서술한다.
    2. 두 번째 문장은 그 영향이 사용자의 상황과 어떻게 이어지는지 서술한다.
- 기사와 사용자 정보의 관련성이 낮아 두 번째 문장이 억지 연결이 되는 경우에는 첫 번째 문장만 작성한다.
- 관련성이 낮은 경우에는 사용자 정보를 억지로 연결하거나 언급하지 않는다.
- 어떤 경우에도 2문장을 넘기지 않는다.
</effect 작성 규칙>
...
- 확인 대상으로 제시할 수 있는 것은 다음 세 가지뿐이다.
    1. 기사 본문이나 <참고 자료>에 실제로 등장한 기관, 제도, 지표, 기업, 문서
...
- 기사에 없는 수치, 날짜, 제도명, 지표명을 새로 만들지 않는다.
</solution 작성 규칙>

<link_name 작성 규칙>
- solution에서 안내한 확인 창구가 아래 목록에 있으면 link_name에 목록에 적힌 이름을 그대로 적는다.
- 목록에 없으면 link_name은 빈 문자열로 둔다.
- 목록에 있는 이름을 임의로 바꾸거나 목록에 없는 이름을 새로 만들지 않는다.
- 안내한 창구가 여러 개이면 사용자가 가장 먼저 확인해야 할 하나만 고른다.
